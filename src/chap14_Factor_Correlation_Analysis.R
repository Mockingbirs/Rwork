...
###########################################
# Chapter14-1. 요인분석(Factor Analysis) 
###########################################

# 요인분석의 목적
# 1. 자료의 요약 : 변인(수)을 몇 개의 공통된 변인으로 묶음
# 2. 변인 구조 파악 : 변인들의 상호관계 파악(독립성 등)
# 3. 불필요한 변인 제거 : 중요도가 떨어진 변수 제거
# 4. 측정도구 타당성 검증 : 변인들이 동일한 요인으로 묶이는지 여부를 확인

# 전제조건 : 등간척도 or 비율척도(연속형변수), 정규분포, 관찰치 상호독립적/분산 동일
...
data.spss <- as.data.set(spss.system.file('C:/workspaces/Rwork/data/drinking_water.sav', encoded = 'utf-8'))
data.spss
View(data.spss)
...
# [실습] 기술 통계량 구하기
result <- read.csv("C:/workspaces/Rwork/data/product.csv", header=TRUE, encoding='UTF-8')
View(result)
