...
gdp <- GDP_ranking16$GDP #gdp 분리
nation <- GDP_ranking16$Nation #나라이름 분리
gdp

install.packages("stringr")
library(stringr)
num_gdp <- as.numeric(str_replace_all(gdp, ',', '')) #콤마 제거
num_gdp
...
# 막대차트 시각화
barplot(GDP_ranking16$GDP, col = rainbow(16),  #GDP의 값을 16가지 색상
        xlab = '국가(nation)', ylab='단위(달러)', names.arg=nation) #(x축y축)
...
print(z)  #변수
print(z*10) #수식
print("x*y=",z)   #error 변수와 수식만 가능 문자열은 불가능
