...
barplot(chart_data, xlim = c(0, 600),
        ...
        col = rainbow(8),
        main = "2019년도 vs 2020년도 분기별 매출현황 비교",
        horiz = T, space=1.5)   # space 차트 사이의 공간 크기 조절

barplot(chart_data, xlim = c(0, 600),
        ylab = "년도별 분기현황", xlab = "매출액(단위:만원)",
        col = rainbow(8),
        main = "2019년도 vs 2020년도 분기별 매출현황 비교",
        horiz = T, space=1.5, cex.names=0.8) #cex.names 글자 크기 조절
...
title("2019~2020년도 분기별 매출현황") # 그래프 생성 이후에도 title(main)을 넣어 줄 수 있음
