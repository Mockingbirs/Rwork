...
x <- TRUE; y <- FALSE
xor(x, y)   # T T / F F 일 때 FALSE 를 반환 T F / F T 일때 TRUE를 반환
x <- TRUE; y <- TRUE
xor(x, y)
...
# 우선순위는 1.산술 연산자  2.비교연산자  3.논리연산자 순으로 연산이 진행된다.
...
sel
exam[sel,] #선택한 index의 열 전체 정보 확인
...
# 매개변수가 있는 함수 예
f2 <- function(x){             #매개변수에는 모든 자료형이 올 수 있다. 벡터 스칼라 매트릭스 등등.....
         cat("x의 값 = ", x, "\n")
         print(x)
      }
...
mean(data) # NA
mean(data, na.rm = T) # 10.77778   re=remove 제거의 의미 / not available=na
