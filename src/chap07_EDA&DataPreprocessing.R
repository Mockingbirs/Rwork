...
getwd()
setwd("C:/workspaces/Rwork/data")
new_data <- read.csv("new_data2.csv", header = T)
View(new_data)

# 범주형(resident) vs 범주형(gender) 데이터 분포 시각화

## 성별에 따른 거주지역 분포 현황 /table 함수는 빈도수를 보여준다.
...
barplot(gender_resident, beside = T, horiz = F,
        col=rainbow(2),
        legend=row.names(gender_resident),  ##legend 범례를 출력
...
##gender_resident <- table(new_data$resident2, new_data$gender2) 데이터 행열 변경
...
# 5.3 연속형 vs 범주형 vs 범주형 mouking bird
...
user_data <- read.csv('user_data2.csv', header = T)
View(user_data)
...
# 데이터 파일 가져오기
pay_data <- read.csv('pay_data2.csv', header = T)
View(pay_data)
...
pay_price <- dcast(pay_data,user_id ~ pay_method, length) #length 갯수 확인
View(pay_price)
...
library(cvTools)
?cvFolds
cross <- cvFolds(n=6, K=3, R=1, type = "random") #n데이터 갯수 k=fold를 3개로(3분할) 123,123반복 r=1 반복횟수 type=인덱스 설정방식 랜덤 
cross
